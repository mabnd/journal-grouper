...
In an ideal world the lines of each entry would always sit consecutively in the file, share identical communications, and be easy to visually identify. In reality they often do not. Communications can vary between lines of the same entry, a line belonging to one entry might be positioned far away from the rest of its group, the data can contain typos in communications, and some entries may be genuinely incomplete. Some entries also legitimately contain two or more lines that are textually identical — the same account, communication, and amount repeated on purpose, for example because an invoice genuinely carries two separate charges of the same value. Every line in the file was put there for a reason and must be accounted for; none are treated as accidental data-entry errors to be discarded. On top of all this, many entries on the same date can look very similar to each other — for example dozens of phone bill entries all starting with "SONATEL Fa L7..." — making it impossible to use communication matching alone as a grouping signal.
...
French accounting notation uses commas as both thousands separators and decimal separators depending on context. The parser disambiguates by examining the structure of the number: if multiple commas are present and all segments after the first contain exactly three digits, all commas are thousands separators and the number is read as a whole integer value — for example "1,135,000" becomes 1135000. If a single comma is present and the segment after it contains exactly three digits, it is also a thousands separator — "127,119" becomes 127119. If a single comma is present and the segment after it does not contain exactly three digits, it is a decimal separator — "127,50" becomes 127.5. Negative values written with a leading minus sign and a space such as "- 101,695" are also handled. Any line where both debit and credit are zero or missing after parsing is flagged immediately since it carries no accounting information and cannot participate in any balance calculation.
...
If the running group of lines is not yet balanced — meaning its debits do not equal its credits — the next line is always absorbed into the group regardless of anything else. An unbalanced group is by definition incomplete so no boundary can be drawn yet.

If the running group is already balanced, the algorithm examines the communication similarity between the next line and the running group's representative communication. If the similarity is at or above a 95% threshold the group is kept open, because a near-identical communication strongly suggests the next line is a continuation of the same entry. If the similarity is below 95% the current group is closed and a new one begins with the next line.
...
This step examines each balanced group that has more than three lines and mixed communications. It builds communication clusters within the group using a single-pass algorithm: each line joins the first existing cluster whose representative communication matches it above 95% similarity, or starts a new cluster if no existing one matches. If at least two distinct clusters each balance independently on their own, the group is split along those cluster boundaries. Any lines not belonging to an independently balanced cluster are kept together as a remainder and confirmed as one entry if the remainder itself balances.
...
The search is made efficient by a pre-built amount index: a dictionary that maps each rounded net amount to the list of lines carrying that amount. Looking up a single stray line is therefore O(1). For pairs, the algorithm iterates over plausible candidates — lines whose individual amount does not exceed the missing balance — and for each one looks up whether the remainder exists in the index. For triples it does the same one level deeper. Plausible candidates are pre-filtered by amount before any combination loop, keeping those loops small even on large files.
...
The first check confirms that the total number of lines in the output — confirmed lines plus flagged lines — exactly equals the number of lines in the original input. The second check confirms that no input line appears more than once anywhere in the output. The third check confirms that every input line appears somewhere in the output. The fourth check confirms that the sum of all debit amounts and the sum of all credit amounts across the entire output exactly match the corresponding totals from the original input file to within the rounding tolerance.
...
A flagged section at the bottom lists every unresolved line, each with its reason and a confidence score of 0, in original file order (sorted by the line's position in the source, not by reason or date) — since flagged lines accumulate out of file order internally (zero-amount flags happen first across the whole file, then per-date-bucket flags, then orphans last), this final sort restores the order a human reviewer would expect when scanning the source for patterns. Because a flagged line is never part of a multi-line group, it carries the journal, date, and reference fields in full on every row, unlike a confirmed entry's continuation lines.
...
For a CSV journal, the unmatched partner names (with their occurrence counts, classification, and suggested match if any) are written to a separate output file. For an Excel workbook, they are appended instead as one extra sheet in the same output workbook, since a workbook can hold more than one sheet but a CSV cannot. If no clients list is supplied, or the supplied path does not exist, this step is skipped entirely and nothing extra is produced.
...
Some journals use a "running consolidation" convention the algorithm cannot see at all: many individual lines are recorded on the date each transaction actually happened, and are only closed out much later by a single summary line whose amount equals the net of everything recorded since the previous summary — sometimes spanning ten or more lines across a dozen different dates. Because Step 2 partitions strictly by exact date before any balance check ever runs, an entry built this way never has a chance to be seen as one group, and every one of its lines is flagged as incomplete even though the underlying entry is genuinely balanced. This was observed in a petty-cash-style journal where periodic "expenses for the period" lines closed out 3-10 days of unrelated single-line expenses at a time. Detecting this reliably would require a different, date-range-aware step, and is not safe to enable universally — the phrase a human would use to describe such a closing line (e.g. "for the period of X to Y") turns out to be common, ordinary descriptive text in plenty of other journals (utility bills, bank interest charges) where the matching lines all still share one single date, so text alone cannot distinguish the two cases. This is currently left as a known limitation rather than guessed at.
